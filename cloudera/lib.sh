...
function runAllTests() {
  ...
  echo ----
  echo
  #mvn -Pcloudera-clover clean verify clover2:aggregate clover2:clover -fn -f ${_POM} -e findbugs:findbugs checkstyle:checkstyle test ${_MAVEN_FLAGS} -Dtest.excludes.file=
  #mvn -Pcloudera-clover sonar:sonar -f ${_POM}${_MAVEN_FLAGS} -Dtest.excludes.file=
}
...
function runStableTests() {
  ...
  echo ----
  echo
  mvn -Pcloudera-clover clean verify clover2:aggregate clover2:clover -fn -f ${_POM} ${_MAVEN_FLAGS} -Dtest.excludes.file=
  mvn -Pcloudera-clover sonar:sonar -f ${_POM} ${_MAVEN_FLAGS}  -Dtest.excludes.file=
}
...
function setupMavenFlags() {
  ...
  MAVEN_FLAGS="-DjavaVersion=$JAVA_VERSION -DtargetJavaVersion=$TARGET_JAVA_VERSION -Dmaven.test.failure.ignore=true -Dtest.fork.count=${TEST_FORK_COUNT} -Dtest.fork.reuse=${TEST_REUSE_FORKS}"
...
function main() {
  ...
  case ${NAME} in
    ...
    test-stable.sh)
      build pom.xml "${MAVEN_FLAGS}" false ${NO_BUILD}
      runStableTests ${POM} "${MAVEN_FLAGS}" "${MAVEN_OPTS}" "${JAVA_OPTS}" "${CLOUDERA_DIR}/excludes.txt"
      ;;
